...
Field::Field(std::string name, Texture *bg_texture, Vector2i screenDimensions)
    : Field(name) {
  /// MAGIC NUMBERS!
...
// update background with texture, View size with rect, m_vertices with rect as
// well
...
void Field::addCell(Texture *texture, unsigned int x, unsigned int y) {
  ...
  IntRect cell_tex_coords =
      IntRect(temp_cell_tex_size.x * x, temp_cell_tex_size.y * y,
...
void Field::addPlayer(Texture *player_texture, Vector2i pos) {
  ...
  map_events_logger->info("Adding player to field with pos {}x{}", pos.x,
                          pos.y);

  player_0 = std::make_shared<Player>("default_player", player_texture,
                                      IntRect(120, 0, 120, 120));
  player_0->set_current_field(this);

  // create default animation with some magic constants (to be resolved with
  // addition of resources manager)
...
// create player at cell [pos.x, pos.y] with animations files: [idle,
// movement_0]
void Field::addPlayer(std::vector<std::string> animation_filenames,
                      Vector2i pos, Vector2u frame_size) {
  if (pos == Vector2i(-1, -1))
    pos = default_player_pos;
  ...
  player_0 = std::make_shared<Player>(
      "animated_player", p_tex, IntRect(0, 0, frame_size.x, frame_size.y));
  player_0->set_current_field(this);
...
void Field::teleport_to(std::shared_ptr<Player> player) {
  map_events_logger->info("Teleporting player to default position on field {}",
                          name);
...
void Field::teleport_to(Vector2i coords, std::shared_ptr<Player> player) {
  map_events_logger->info("Teleporting player position {}x{} on field {}",
                          coords.x, coords.y, name);
...
void Field::set_player_movement_direction(int direction) {
  ...
  if (player_0 && player_0->get_current_field() == this) {
    ...
    player_0->add_movement_direction(Vector2f(movement_x, movement_y),
                                     direction);
  }
}

// cancel scheduled movement. If direction doesn't match - do nothing. If there
// are few directions scheduled - pop only coinciding
...
void Field::add_object_to_cell(int cell_x, int cell_y, std::string type_name,
                               Texture *texture) {
...
void Field::change_cell_texture(int cell_x, int cell_y, std::string name,
                                Texture *texture) {
...
void Field::update_view_center() {
  Vector2f view_center =
      Vector2f(cell_center_x * cell_length_x, cell_center_y * cell_length_y);
...
// field_block provides textures for cells by names (instead of resources
// manager)
void Field::load_field(std::map<std::string, Texture *> &field_block,
                       int loc_id) {
  ...
  // get field size
  unsigned int field_length = int(Location["scale"][0].template get<int>());
  unsigned int field_width = int(Location["scale"][1].template get<int>());

  loading_logger->info("Loading field with size of {}x{}", field_length,
                       field_width);

  default_player_pos.x = Location["default_player_pos"][0].template get<int>();
  default_player_pos.y = Location["default_player_pos"][1].template get<int>();
  field_resize(field_length, field_width);

  cell_tex_size =
      Vector2u(Location["cell_texture_size"][0].template get<int>(),
               Location["cell_texture_size"][0].template get<int>());

  for (unsigned int x = 0; x < cells.size(); x++) {
    for (unsigned int y = 0; y < cells[x].size(); y++) {
      // add cell
      std::string cell_type =
          Location["map"][x][y]["type"].template get<std::string>();
      ...
      IntRect cell_tex_coords =
          IntRect(temp_cell_tex_size.x * x, temp_cell_tex_size.y * y,
                  temp_cell_tex_size.x, temp_cell_tex_size.y);
      cells[x][y] =
          new Cell(cell_type, field_block[cell_type], cell_tex_coords);
      ...
      if (Location["big_objects"][x][y].is_array()) {
        ...
        for (unsigned int i = 0; i < cell_object_size; i++) {
          if (Location["big_objects"][x][y][i].is_object()) {
            std::string object_type = Location["big_objects"][x][y][0]["type"]
                                          .template get<std::string>();
            // std::string object_depth_level =
            //     Location["big_objects"][x][y][0]["depth_level"]
            //         .template get<std::string>();
...
void Field::draw(RenderTarget &target, RenderStates states) const {
  ...
  // draw player atop of everythinghas current field? {}",
  // (player_0->get_current_field() == this));
  if (player_0 && player_0->get_current_field() == this) {
...
Field *new_field(Texture *bg, unsigned int cell_length, unsigned int cell_width,
                 Texture *cell_texture, Texture *player_texture,
                 Vector2i screen_dimensions) {
  Field *field = new Field("Main field");
  field->addTexture(bg, IntRect(0, 0, 1920, 1080));
  field->setScale((float)screen_dimensions.x / 1920,
                  (float)screen_dimensions.y / 1080);
